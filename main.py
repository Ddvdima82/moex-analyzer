...
def main() -> None:
    ...
    try:
        ...
        # Самокалибровка весов WEIGHTS по накопленной статистике прогонов —
        # неблокирующая: тротлинг/минимум наблюдений внутри, ошибка не валит
        # прогон. Считанные веса уйдут в дело со СЛЕДУЮЩЕГО прогона (WEIGHTS
        # читается при импорте config, не хот-релоадится посреди этого).
        try:
            from calibration import compute_and_save_calibration
            compute_and_save_calibration()
        except Exception as exc:
            logger.error("Ошибка самокалибровки весов: %s", exc)
