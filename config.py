...
# Максимальный возраст закэшированной ключевой ставки ЦБ (дни). При
# недоступности cbr.ru берём последнее известное значение: ставка меняется
# на заседаниях (~раз в 6-7 недель), поэтому вчерашняя куда ближе к правде,
# чем None (при None фундаментал молча теряет обе поправки на ставку).
# 120 дней ≈ два пропущенных заседания — дальше значению уже нельзя доверять.
CBR_RATE_MAX_AGE_DAYS: int = 120
...
# Автообновляемый слой фундаментала (data/fundamentals_parser.py) — генерируемый
# артефакт класса calibration.json: гитигнорится, кэшируется в CI. Накладывается
# ПОЛЕВО поверх ручного fundamentals.json, который остаётся фолбэком и
# единственным источником полей, которых нет у парсера (сектор, рост выручки).
FUNDAMENTALS_AUTO_FILE: pathlib.Path = BASE_DIR / "data" / "fundamentals_auto.json"
# Как часто ходить к источнику: отчётность квартальная, чаще незачем.
FUNDAMENTALS_AUTO_MAX_AGE_DAYS: int = 7
