...
def score_technical(indicators: dict) -> float:
    ...
    # ВАЖНО: гистограмма выражена в цене актива (рубли), поэтому нормируем на
    # текущую цену → безразмерное отношение. Иначе дорогие бумаги уносит к
    # экстремуму, а дешёвые жмёт к 0.5 при одинаковом %-моментуме.
    # Масштаб 0.005 = 0.5% цены → atan(1) → ~0.75. Нулевой MACD → 0.5.
    macd_hist = indicators.get("macd_histogram", 0.0)
    price = indicators.get("current_price", 0.0) or 0.0
    macd_ratio = (macd_hist / price) if price > 0 else 0.0
    macd_score = math.atan(macd_ratio / 0.005) / math.pi + 0.5
    score += 20 * macd_score
    ...
    # 5. Позиция в 52w (вес 15) — контрарно-доминантное смешение режимов.
    # При низком RSI ценим близость к лоу (отскок), при высоком — к хаю (моментум,
    # сдержанно ×0.5). Контрарная компонента доминирует — это даёт лучший
    # walk-forward edge на голубых фишках MOEX (mean-reversion: чисто моментумный
    # режим проигрывает на VTBR/SBER/MTSS). RSI здесь и в п.1 — сознательная связь,
    # подтверждённая бэктестом, а не дубль.
